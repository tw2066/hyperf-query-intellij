...
class DatabaseUtils private constructor() {
    companion object {
        ...
        private fun DasTable.isPrefixed(project: Project): Boolean =
            this.name.startsWith(HyperfQuerySettings.getInstance(project).tablePrefix)

        fun DasTable.nameWithoutPrefix(project: Project): String =
            this.name.substringAfter(HyperfQuerySettings.getInstance(project).tablePrefix)

        fun DasColumn.tableNameWithoutPrefix(project: Project): String =
            this.tableName.substringAfter(HyperfQuerySettings.getInstance(project).tablePrefix)
