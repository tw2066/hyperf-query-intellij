...
### 文档
- 新增 `docs/` 详细使用文档:快速入门(安装、数据源配置、排错速查)、功能详解(每项功能配 PHP 示例)、相关配置(设置项与 `databases.php` 优先级互动)、适配性(版本要求、支持方法清单、已知限制);README 加入口链接

## 1.1.1 - 2026-09-08

### 新增
- 设置面板顶部新增全局开关 **Enable Hyperf Query**(默认开启,按项目保存);关闭后补全、未知元素检查、Ctrl+Click 引用解析与悬停文档等全部插件功能停用,面板其余选项联动禁用

### 修复
- 检测到 Hyperf 项目但未配置数据源过滤时的引导通知不再每次打开项目都弹出,改为每个项目最多显示一次(此前只有点击 Ignore 才不再提醒,点关闭按钮无效);插件停用期间该通知也不再弹出

- 修复项目设置因修改计数未更新而无法写入 `.idea/hyperf-query-settings.xml` 的问题;首次提示和点击 **Ignore** 后现在会立即调度保存,不再每次打开项目都重复提示
...
- `where(['id' => 1, 'goods_name' => 2])` 关联数组写法中，从第二个键起的列名现在可以正常补全和 Ctrl+Click 跳转。此前参数位计算误将数组内序号当作参数序号，导致只有第一个键生效
- `where(['id' => 2, ''])` 数组顶层裸值（正在输入新条件时）现在也会触发列名补全
- 模型 `$table` 属性默认值的悬停文档改为与方法内表名参数一致的轻量摘要（schema.表名 + 注释），不再渲染完整 DDL 造成卡顿
